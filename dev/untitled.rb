...
- # Сервер tg: ruby app/tg_server.py    (подлючается к telegram-cli, позволяя писать сообщения боту. Делает все запросы сам через эвент отправки сообщения)
! # Цикл request: rake request          (делает каждый 5 секунд запросы) - отказ в пользу сервера tg
# 
# 
