...
Проблемы:
+ Рефакторинг кода
+ Упаковать все в докер (файлы telegram-cli заранее скомпилировать и потом добавлять через гит в докер)
0 возвращать в редис ссылки в случае ошибки (подумать нужно ли это вообще)
  # поскольку обработки ошибок нет, то при первом же неудачном запросе все данные в редисе будут потеряны
  # (они выгребаются сначала, а только потом отсылаются)
+ сделать ENV['TOKKEN'] - передается в докер с сервера
...
0 Корпоративные стандарты
  ...
  + CI нет (автосборка докер образа)
  0 тестов нет (тестировать только мой код)
  1 форматирования кода(rubocop) нет
  0 деплоя нет
  0 ...
